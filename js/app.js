...
function buildLoadRows(){
  ...
  DB.loads
    ...
    .forEach(l=>{
      ...
      if(!existing.has(key))existing.set(key,{analyst_id:l.analyst_id,project_id:l.project_id,hours:{},dirty:{}});
  ...
  DB.assignments.filter(a=>a.analyst_id&&a.project_id&&activeProjectIds.has(a.project_id)).forEach(a=>{
    ...
    rows.set(key, existing.get(key)||{analyst_id:a.analyst_id,project_id:a.project_id,hours:{},dirty:{}});
  });
...
function setLoadHour(i,wid,val){
  if(!loadRows[i])return;
  loadRows[i].hours[wid]=Math.max(0,Math.round(num(val)));
  if(!loadRows[i].dirty)loadRows[i].dirty={};
  loadRows[i].dirty[wid]=true;
}
    ...
    .forEach(r=>{
      Object.keys(r.dirty||{}).forEach(weekId=>{
        ...
        changes.push({
          ...
          week_id:weekId,
          planned_hours:Math.max(0,Math.round(num(r.hours[weekId]||0)))
        });
      });
    });
...
async function saveProject(){
  ...
  if(!ok)return;
  closeProjectModal();
  // Al crear un proyecto limpiamos únicamente los filtros de Proyectos para
  // que el registro recién creado sea visible inmediatamente. No se toca el Dashboard.
  if(!id){
    projectFilterState.analysts.clear();
    projectFilterState.statuses.clear();
    projectFilterState.countries.clear();
...
function reportStatusClass(status){
  const st=normalizeProjectStatus(status).toLowerCase();
  if(st.includes('final'))return 'status-finalizado';
  if(st.includes('ejec')||st.includes('proceso')||st==='activo')return 'status-ejecucion';
  if(st.includes('pausa'))return 'status-pausa';
  if(st.includes('asign'))return 'status-asignado';
  if(st.includes('cancel')||st.includes('bloque'))return 'status-alerta';
  return 'status-neutral';
}
function reportStatusBadge(status){return `<span class="report-status-badge ${reportStatusClass(status)}">${esc(status||'Sin estado')}</span>`}
function renderReportStatusChart(projects){
  ...
  box.innerHTML=Object.entries(counts).sort((a,b)=>b[1]-a[1]).map(([st,n])=>`<div class="report-bar-row ${reportStatusClass(st)}"><span>${reportStatusBadge(st)}</span><div class="report-bar-track"><i style="width:${n/max*100}%"></i></div><b>${n}</b></div>`).join('')||'<small>Sin proyectos para los filtros seleccionados.</small>';
}
